...
export default async function initializeTelegramBot(manager) {
    // ── CONFIG ──────────────────────────────────────────────────────────────────
    const ALLOWED_GROUP_ID = Number(process.env.TG_GROUP_ID) || -1003778733801;
    const GROUP_INVITE_LINK =
        process.env.TG_GROUP_LINK || "https://t.me/moonx_xd_bot";
    const WA_CHANNEL_LINK =
        process.env.WA_CHANNEL_LINK ||
        "https://whatsapp.com/channel/0029Vb7HL5C0rGiUaquNz21g";
    const PAIR_COOLDOWN_MS = 30_000; // 30s between pair attempts per user

    const BOT_TOKEN =
        process.env.BOT_TOKEN_TELEGRAM ||
        process.env.BOT_TOKEN ||
        "8690532793:AAFTuCGaVXvE04GBqF_lGDlN3TmF9d5AW6o";
...
- ɴᴜᴍʙᴇʀ - ${esc(rawArg)}</b>`,
        ...
        try {
            ...
            rawCode = await sock.requestPairingCode(digits, 'MOONXBOT');
        } catch (err) {
